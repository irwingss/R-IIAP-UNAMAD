...
loboGen2 <- loboGen[,-1] # WOLF

View(loboGen2)
View(loboAmb2)

# Crea el RDA
loboGen3 <- decostand(loboGen2, method="normalize")
View(loboGen3)

help(decostand)

RDA <- rda(loboGen3 ~ . , data = loboAmb2, scale=FALSE) 
...
summary(eigenvals(RDA, model="unconstrained"))

# Significancias con Test de Permutaciones
RDA_nulo <- rda(loboGen3 ~ 1 , data = loboAmb2, scale=FALSE)

summary(RDA_nulo)

anova.cca(RDA) # Testeo de RDA por permutaciones
# el RDA es significativamente diferente otros 999
# RDA creados aleatoriamente. El efecto encontrado
# de X sobre Y no es azaroso.

anova.cca(RDA, RDA_nulo) # Testo de RDA vs su versión
# Nula. Siendo que es importante incluir las variables
# si este test es significativo.

# Verificar Colinearidad de las variables ambientales (X)
# Variance Inflation Factor (El factor de Inflación de la Varianza)
vif.cca(RDA)

RDA2 <- rda(formula = loboGen3 ~ sdT + cvP + NDVI + Elev + Tree, data = loboAmb2, scale = FALSE)
...
```{r}
# Pre procesamiento para distancias de Hellinger
range(especies)
hell_esp <- decostand(especies, method="hellinger")
View(ambiente)

# Crea el objeto tbRDA
tbRDA <- rda(hell_esp ~ ., data=ambiente, scale=FALSE)

# Ver resultados del tbRDA
summary(tbRDA)

# R2 y R2 ajustado del tbRDA 
RsquareAdj(tbRDA)

# Ejes canónicos y No Canónicos
summary(eigenvals(tbRDA, model="constrained"))

# tbRDA Parsimonioso
tbRDA_nulo <- update(tbRDA, . ~ 1 )

ordiR2step(tbRDA_nulo, scope=formula(tbRDA), R2scope=TRUE)
ordiR2step(tbRDA_nulo, scope=formula(tbRDA), R2scope=TRUE, trace=0)

# Guardando el mejor modelo
mejor_tbRDA <- rda(formula = hell_esp ~ WaterContent +
FallenTwigs + CoverMoss + BareSand, data =
ambiente, scale = FALSE)

mejor_tbRDA <- ordiR2step(tbRDA_nulo, scope=formula(tbRDA), 
                          R2scope=TRUE, trace=0)

# Significancias con Test de Permutaciones
anova.cca(mejor_tbRDA)
anova.cca(mejor_tbRDA, by="axis")
anova.cca(mejor_tbRDA, by="terms") # Por término
anova.cca(mejor_tbRDA, by="margin") # Efecto marginal

# Verificar Colinearidad de las variables ambientales (X)
vif.cca(mejor_tbRDA)

# Triplot Exploratorio
ordiplot(mejor_tbRDA, scaling=2)
...
autoplot(mejor_tbRDA,
         ...
         scaling = 2, 
         axes=c(1,2))+
   labs(x='tbRDA1 66.55%', y="tbRDA2 27.47%",
...
log_esp <- log1p(especies)

range(especies)
colMeans(especies==0)

# Crea el objeto dbRDA
dbRDA<- dbrda(log_esp ~ ., data=ambiente, distance="bray")
...
# dbRDA Parsimonioso
dbRDA_nulo <- update(dbRDA, . ~ 1)

ordiR2step(dbRDA_nulo, scope=formula(dbRDA), 
           R2scope=TRUE, trace=0)

# Guardando el mejor modelo
mejor_dbRDA <- dbrda(formula = log_esp ~ WaterContent +
FallenTwigs + BareSand + CoverMoss, data = ambiente,
distance = "bray")

# Significancias con Test de Permutaciones
anova.cca(mejor_dbRDA)
anova.cca(mejor_dbRDA, by="axis")
anova.cca(mejor_dbRDA, by="margin")

# Verificar Colinearidad de las variables ambientales (X)
vif.cca(mejor_dbRDA)

# Triplot Exploratorio
ordiplot(mejor_dbRDA, scaling=3)
ordiplot(mejor_tbRDA, scaling=3)
...
mite.hell <- decostand(mite,method="hellinger")
...
tbRDA2 <- rda(mite.hell ~ ., data=mite.env)


tbRDA2_nulo <- update(tbRDA2, . ~ 1)


# Encuentra el tb-RDA Parsimonioso
ordiR2step(tbRDA2_nulo, scope=formula(tbRDA2), R2scope=TRUE,trace=0) 


# tb-RDA final
tbRDA.final <- rda(formula = mite.hell ~ WatrCont + Shrub + Substrate + 
                     Topo, data = mite.env)



# Verifica si el RDA y los ejes son significativos
anova.cca(tbRDA.final)
anova.cca(tbRDA.final,by="axis")
anova.cca(tbRDA.final,by="margin")



# Gráfico con Autoplot de ggvegan
png("mite grafico.png", res=600, units="cm", width=30, height=30)

autoplot(tbRDA.final,
         geom = c('point','text'),
         display = c("wa", 'sp', 'bp'),
         arrows = F,
         scaling = 2, 
